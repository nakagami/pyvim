...
def tab_expand(editor):
    ...
    editor.current_editor_buffer.expand_tab = True
...
def tab_no_expand(editor):
    ...
    editor.current_editor_buffer.expand_tab = False
...
def tab_stop(editor, value):
    ...
    if value is None:
        ...
        try:
            value = int(value)
            if value > 0:
                editor.current_editor_buffer.tabstop = value
            else:
...
def shift_width(editor, value):
    ...
    if value is None:
        ...
        try:
            value = int(value)
            if value > 0:
                editor.current_editor_buffer.shiftwidth = value
            else:
                editor.show_message('Argument must be positive')
        except ValueError:
            editor.show_message('Number required after =')
