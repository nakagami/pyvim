...
HighlightSearchProcessor.apply_transformation = (
    _highlight_search_processor_apply_transformation
)
...
class WelcomeMessageWindow(ConditionalContainer):
    ...
    def __init__(self, editor):
        ...
        def condition():
            ...
            result = (
                len(buffers) == 1
                and buffers[0].buffer.text == ""
                and buffers[0].location is None
                and not once_hidden[0]
            )
            if not result:
                once_hidden[0] = True
            return result
        ...
        def get_tokens():
            ...
            if len(buffer_infos) == 0:
                ...
                for info in buffer_infos:
                    ...
                    result.extend(
                        highlight_location(eb.get_display_name(), search_string, t)
                    )
                    result.extend(
                        [
...
class WindowStatusBar(FormattedTextToolbar):
    ...
    def __init__(self, editor, editor_buffer):
        ...
        super(WindowStatusBar, self).__init__(get_text, style="class:toolbar.status")
    ...
    def __init__(self, editor, window_arrangement):
        ...
        self._fc = FloatContainer(
            ...
            floats=[
                ...
                Float(bottom=1, left=0, right=0, height=1, content=ValidationToolbar()),
                Float(
                    bottom=1,
                    left=0,
                    right=0,
                ...
                Float(
                    content=WelcomeMessageWindow(editor),
        ...
        self.layout = Layout(
            FloatContainer(
                ...
                floats=[
                    Float(
                        right=0, height=1, bottom=0, width=5, content=SimpleArgToolbar()
                    ),
                ],
            )
        )
